...
VERSION="${CHATAY_SEMVER:-v2.14.6-alpha.1}"
REPLICA_COUNT="${HS1_REPLICA_COUNT:-4}"
CLIENT_PROCESS_COUNT="${HS1_CLIENT_PROCESS_COUNT:-1}"
BASE_PORT="${HS1_BASE_PORT:-22000}"
READY_TIMEOUT_SEC="${HS1_READY_TIMEOUT_SEC:-90}"
CLIENT_TIMEOUT_SEC="${HS1_CLIENT_TIMEOUT_SEC:-45}"
START_STAGGER_SEC="${HS1_START_STAGGER_SEC:-0.5}"
AFTER_SET_SLEEP_SEC="${HS1_AFTER_SET_SLEEP_SEC:-2}"
BAZEL_JOBS="${BAZEL_JOBS:-4}"
OPERATION_COUNT="${HS1_OPERATION_COUNT:-1}"
RUN_ID="${RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-hs1-kv}"
...
SERVER_BIN="${ROOT}/bazel-bin/benchmark/protocols/hotstuff/kv_service"
CLIENT_BIN="${ROOT}/bazel-bin/service/tools/kv/api_tools/kv_service_tools"
TEST_KEY="${HS1_TEST_KEY:-chatay-hs1-${RUN_ID}}"
TEST_VALUE="${HS1_TEST_VALUE:-chatay-hs1-value-${RUN_ID}}"
...
cleanup() {
  if [[ "${HS1_KEEP_CLUSTER:-0}" == "1" ]]; then
    log "HS1_KEEP_CLUSTER=1; leaving ${#pids[@]} processes running"
    write_manifest || true
    return
  fi
  for pid in "${pids[@]:-}"; do
...
log "normalizing client config as RegionInfo JSON"
...
log "starting ${TOTAL_PROCESS_COUNT} hs1 kv_service processes"
...
error_code=""
log "HS1 KV warm-cluster passed operations=${passed_operations}/${OPERATION_COUNT}"
